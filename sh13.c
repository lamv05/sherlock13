...
void *fn_serveur_tcp(void *arg)
{
       ...
       if (bind(sockfd, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0)
        ...
        while (1)
        {
                ...
                //Le thread réseau se bloque jusqu'à ce que la boucle graphique le remette à zéro
				while (synchro);
...
int main(int argc, char ** argv)
{
    ...
    // Partie logicielle de l'interface graphique qui gère le double buffer (on crée la prochaine image dans un buffer, puis on commute à l'image actuelle)
   ...
   ret = pthread_create ( & thread_serveur_tcp_id, NULL, fn_serveur_tcp, NULL);
    ...
    while (!quit)
    {
	// Récupère un event, fonction non bloquante
	if (SDL_PollEvent(&event))
	{
        	switch (event.type)
        	{
				...
				case  SDL_MOUSEBUTTONDOWN:
					...
					else if ((mx>=500) && (mx<700) && (my>=350) && (my<450) && (goEnabled==1))
					{
						...
						{
							//Demande de symbole à tous les joueurs
						...
						{
							//Demande à un joueur de son nombre de tel symbole
		...
		switch (gbuffer[0])
		{
			...
			case 'L':
				...
				printf("Liste des joueurs connectés : %s %s %s %s\n",gNames[0],gNames[1],gNames[2],gNames[3]);

				break;
			// Message 'D' : le joueur recoit ses trois cartes
			case 'D':
				// RAJOUTER DU CODE ICI
				sscanf(gbuffer+ 2, "%d %d %d %d %d %d %d %d %d %d %d",&b[0],&b[1],&b[2],&tableCartes[gId][0],&tableCartes[gId][1],&tableCartes[gId][2],&tableCartes[gId][3],&tableCartes[gId][4],&tableCartes[gId][5],&tableCartes[gId][6],&tableCartes[gId][7]);
				printf("Cartes reçues: %s \n%s \n%s\n",nbnoms[b[0]],nbnoms[b[1]],nbnoms[b[2]]);
			...
			case 'M':
				// RAJOUTER DU CODE ICI
				int currentPlayer;
				sscanf(gbuffer+ 2, "%d",&currentPlayer);
				printf("Joueur courant : %s\n",gNames[currentPlayer]);
				if(currentPlayer == gId)
					goEnabled = 1;
				else
					goEnabled = 0;
			...
			case 'V':
				// RAJOUTER DU CODE ICI
				int temp, joueurTarg, objectTarg;
				sscanf(gbuffer+2, "%d %d %d", &temp, &joueurTarg, &objectTarg);
				// Si une question a été posée sur le fait d'avoir ou non un objet, on n'affecte pas sa propre ligne
				// 42 signifie que le joueur possède l'objet
				if(gId != joueurTarg)
					tableCartes[joueurTarg][objectTarg] = temp;
				break;
			// Fin de la partie
			case 'E':
				printf("%s\n",gbuffer+2);
