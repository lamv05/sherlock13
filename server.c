...
char *nameobjets[]={"Pipe","Ampoule","Poing","Insigne","Cahier","Collier","Oeil","Crâne"};
int joueurCourant;
...
void createTable()
{
	...
	// Création de la matrice complète (grille complète, avec les symboles de chaque joueur)
	for (i=0;i<4;i++)
		for (j=0;j<8;j++)
			tableCartes[i][j]=0;
        ...
        if (fsmServer==0)
        {
        	switch (buffer[0])
        	{
				case 'C':
					...
					tcpClients[nbClients].playing = 1;
					nbClients++;
					...
					if (nbClients==4)
					{
						...
						sprintf(reply,"D %d %d %d %d %d %d %d %d %d %d %d", deck[9], deck[10], deck[11], tableCartes[3][0],tableCartes[3][1],tableCartes[3][2],tableCartes[3][3],tableCartes[3][4],tableCartes[3][5],tableCartes[3][6],tableCartes[3][7]);
		...
		else if (fsmServer==1)
		{
			...
			switch (buffer[0])
			{
				...
				case 'G':
					...
					else
					{
						...
						else
						{
							...
							broadcastMessage(reply);
							tcpClients[joueurCourant].playing = 0;
							do {
								if(joueurCourant == 3)
									joueurCourant = 0;
								else
									joueurCourant++;
							} while(!tcpClients[joueurCourant].playing);
				...
				case 'S':
					...
					sprintf(reply,"%s a demandé à %s combien il avait d'objets \"%s\"\n",tcpClients[id].name,tcpClients[joueurSel].name,nameobjets[objSel]);
					broadcastMessage(reply);
